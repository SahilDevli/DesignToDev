...
async function main(): Promise<void> {
  ...
  // Snapshot before writing, so only OUR edit reaches the branch.
  const before = NO_PR || DRY_RUN ? new Set<string>() : changedPaths();
  ...
  if (!NO_PR) {
    const paths = attributePaths(before, changedPaths(), before, GIT_CFG);
    runGitFlow({
      cfg: GIT_CFG,
      blocked: [],
      units:
