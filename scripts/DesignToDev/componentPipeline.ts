...
interface BuildOutcome {
  ...
  docsFailed: number;
  usage: UsageRow[];
  /** One PR unit per component that actually produced file changes. */
  units: PrUnit[];
  /** Components whose worst drift reached the gate — these block the whole run. */
...
async function runBuildPhase(
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    // component's work, and lands on its own branch.
    ...
    // ── What this component actually wrote, for its own branch and PR ──
    if (!NO_PR) {
      const paths = attributePaths(before, changedPaths(), PREEXISTING, GIT_CFG);
      if (paths.length === 0) {
        log("ℹ️ ", `Component ${c.name} — no file changes to ship`);
        ...
        out.units.push(prUnitFor(c, mode, paths, worst, model));
        log("📦", `Component ${c.name} — ${paths.length} file(s) ready to ship`);
      }
    }
  }
